...
import { generateWidgetIdentifier } from "../widgetIdentifierCounter.js";
...
// PropertyGridWidget must be provided by parent (DashboardDesignerView)
const openPropertyEditor = inject('openPropertyEditor', null);
if (!openPropertyEditor) {
  console.warn('DashboardDesignerWidget: openPropertyEditor not provided. PropertyGridWidget may not work for widgets.');
}

// WidgetsSelectorWidget must be provided by parent (DashboardDesignerView)
...
if (!openWidgetsSelector) {
  console.warn('DashboardDesignerWidget: openWidgetsSelector not provided. WidgetsSelectorWidget may not work.');
}
...
function getDefaultPropsForWidgetType(type) {
  switch (type) {
    ...
    case "GridstackWidget":
      return {
        identifier: generateWidgetIdentifier("GridstackWidget"),
        widgets: [],
        cellHeight: '70px',
        cellWidth: '120px',
      };
...
function openWidgetsSelectorHandler(regionId) {
  ...
  // Call injected openWidgetsSelector function
  if (openWidgetsSelector) {
    const info = getColumnInfoFromRegionId(regionId);
    if (widgetsByGroup.value) {
      openWidgetsSelector(widgetsByGroup.value, info || null);
    } else {
      console.warn('DashboardDesignerWidget: Cannot open WidgetsSelector - widgetsByGroup is missing', { widgetsByGroup: widgetsByGroup.value });
    }
  } else {
    console.warn('DashboardDesignerWidget: openWidgetsSelector not provided');
  }
}
