...
export const widgetMeta = {
  ...
  description: 'Displays a label, icon, and value (e.g. metric or KPI) with configurable colors and fonts.',
...
export const PROP_SCHEMA = {
  ...
  icon: { type: 'string', default: 'fa-warning', control: 'text', label: 'Icon' },
  iconStyle: { type: 'string', default: 'fa', control: 'select', options: ['fa', 'fas', 'far', 'fab'], label: 'Icon style' },
  iconColor: { type: 'string', default: 'orange', control: 'colorPure', label: 'Icon color' },
  iconSize: { type: 'string', default: 'fa-3x', control: 'select', options: ['fa-1x', 'fa-2x', 'fa-3x', 'fa-4x', 'fa-5x', 'fa-lg', 'fa-sm'], label: 'Icon size' },
  iconHorizontalAlignment: { type: 'string', default: 'start', control: 'select', options: ['start', 'center', 'end'], label: 'Icon alignment' },
  iconAnimation: { type: 'string', default: '', control: 'select', options: ['', 'spin', 'pulse', 'beat', 'fade', 'bounce', 'shake'], label: 'Icon animation' },
  iconAnimationSpeed: { type: 'string', default: 'normal', control: 'select', options: ['normal', 'slow', 'slower'], label: 'Animation speed' },
...
const CONFIGURABLE_PROPS_BY_GROUP = {
  ...
  icon: ['icon', 'iconStyle', 'iconColor', 'iconSize', 'iconHorizontalAlignment', 'iconAnimation', 'iconAnimationSpeed'],
...
export function buildPropertySchema(props = {}) {
  const displayLabel = props.identifier || props.label || props.value || 'Icon value'
...
const props = defineProps({
  ...
  icon: { type: String, default: 'fa-warning' },
  iconStyle: {
    type: String,
    default: 'fa',
    validator: (v) => ['fa', 'fas', 'far', 'fab'].includes(v),
  },
  ...
  iconSize: {
    type: String,
    default: 'fa-3x',
    validator: (v) => ['fa-1x', 'fa-2x', 'fa-3x', 'fa-4x', 'fa-5x', 'fa-lg', 'fa-sm'].includes(v),
  },
  ...
  iconAnimation: {
    type: String,
    default: '',
    validator: (v) => ['', 'spin', 'pulse', 'beat', 'fade', 'bounce', 'shake'].includes(v),
  },
  iconAnimationSpeed: {
    type: String,
    default: 'normal',
    validator: (v) => ['normal', 'slow', 'slower'].includes(v),
  },
  horizontalAlignment: {
    type: String,
    default: 'center',
...
const iconClasses = computed(() => {
  const c = [props.iconStyle, props.iconSize, props.icon, 'mb-0']
  if (props.iconAnimation) c.push(`fa-${props.iconAnimation}`)
  return c
})

const animationDurationMap = { normal: '1s', slow: '1.5s', slower: '2s' }
const iconStyle = computed(() => {
  const style = { color: props.iconColor }
  if (props.iconAnimation && props.iconAnimationSpeed) {
    style.animationDuration = animationDurationMap[props.iconAnimationSpeed] || '1s'
  }
  return style
})
...
const iconAlignmentClass = computed(() => `align-self-${props.iconHorizontalAlignment}`)

// Widget-level alignment styles (similar to GridLayoutWidget)
  ...
  >
    <div :class="bodyClasses" :style="contentAlignmentStyle">
      ...
      <div class="d-flex flex-column min-w-0 flex-grow-1 w-100">
        <p class="icon-value fw-bold mb-0" :class="[valueFontSize, valueAlignmentClass]" :style="valueStyle">{{ value }}</p>
        <p v-if="label" class="icon-label text-muted mb-0" :class="[labelFontSize, labelAlignmentClass]" :style="labelStyle">{{ label }}</p>
      </div>
    </div>
  </div>
</template>
