...
const widgetMeta = {
  friendlyName: "Tile Layout",
  description: "Nested tile layout (rows and columns).",
...
/** Clear selectedRegionId in all cells that contain a nested TileLayoutWidget (closes their offcanvases). */
function clearNestedTablesSelection() {
  const newRows = localModel.value.rows.map((row) => {
    const newColumns = (row.columns || []).map((col) => {
      if (col?.content?.type === "TileLayoutWidget" && col?.content?.props) {
        return {
          ...col,
          content: {
            ...col.content,
...
/** When a nested TileLayoutWidget emits update:modelValue, sync rows and selection into the cell's content.props. */
...
/** Key for nested TileLayoutWidget so Vue re-renders it when row data (e.g. heights) changes and the child table updates. */
...
if (!openPropertyEditor) {
  console.warn(
    "TileLayoutWidget: openPropertyEditor not provided. PropertyGridWidget may not work for widgets.",
  );
}
...
if (!openWidgetsSelector) {
  console.warn(
    "TileLayoutWidget: openWidgetsSelector not provided. WidgetsSelectorWidget may not work.",
  );
}
...
/** Which settings panel to show when a cell is selected: 'column' (Column Settings) or 'cell' (Cell Settings). */
const selectedSettingsPanel = ref(null);
...
/** True when a cell is selected and Cell Settings panel should be shown. */
...
const previewCanvasStyle = computed(() => ({
  ...
  minWidth: isNestedTable ? 0 : undefined,
  display: "grid",
...
function getCellStyle(row, colIndex) {
  ...
  if (!editMode.value) {
    ...
    if (selected === rowRegionId) {
      // Selected row: Bootstrap danger red on all cells in the row
      style.border = "2px solid #dc3545";
      style.boxShadow = "0 0 0 0.2rem rgba(220, 53, 69, 0.25)";
    } else if (
      isColumnSelected.value &&
      selectedSettingsPanel.value === "column" &&
      getSelectedColumnIndex() === colIndex
    ) {
      // Column Settings open: Bootstrap primary blue on all cells in the column (same colIndex across rows)
      style.border = "2px solid #0d6efd";
      style.boxShadow = "0 0 0 0.2rem rgba(13, 110, 253, 0.25)";
    } else if (selected === cellRegionId) {
      // Cell Settings open: blue on this cell only
      style.border = "2px solid #0d6efd";
      style.boxShadow = "0 0 0 0.2rem rgba(13, 110, 253, 0.25)";
    } else {
...
function getDefaultPropsForWidgetType(type) {
  switch (type) {
    ...
    case "TileLayoutWidget":
      return {
        identifier: generateWidgetIdentifier("TileLayoutWidget"),
        rows: [
          {
            id: generateRowId(),
...
function openWidgetsSelectorHandler(regionId) {
  ...
  if (openWidgetsSelector) {
    ...
    if (widgetsByGroup.value) {
      ...
      console.warn(
        "TileLayoutWidget: Cannot open WidgetsSelector - widgetsByGroup is missing",
        { widgetsByGroup: widgetsByGroup.value },
      );
    }
  } else {
    console.warn("TileLayoutWidget: openWidgetsSelector not provided");
  }
}
...
function selectRegion(regionId, panel) {
  // Close Widgets Selector offcanvas when opening Row or Column/Cell Settings
  ...
  if (isOpening) {
    const isRowId = !/^.+-col-\d+$/.test(regionId);
    selectedSettingsPanel.value = isRowId ? "row" : panel ?? "column";
  } else {
    selectedSettingsPanel.value = null;
  }

  let updates = { selectedRegionId: newSelectedRegionId };
  if (isOpening) {
    const hasNestedTables = localModel.value.rows?.some((row) =>
      row.columns?.some((col) => col?.content?.type === "TileLayoutWidget"),
    );
    if (hasNestedTables) {
      updates.rows = localModel.value.rows.map((row) => ({
        ...
        columns: (row.columns || []).map((col) => {
          if (
            col?.content?.type === "TileLayoutWidget" &&
            col?.content?.props
          ) {
            return {
              ...col,
...
/** Remove the cell only in the selected row (not from all rows). */
function removeCell(rowIndex, colIndex) {
  ...
  const newRows = localModel.value.rows.map((r, i) => {
    ...
    const cols = r.columns && Array.isArray(r.columns) ? [...r.columns] : [];
    if (colIndex < cols.length) cols.splice(colIndex, 1);
    return { ...r, columns: cols.length ? cols : undefined };
  });
  const row = newRows[rowIndex];
  const newColIndex = Math.min(colIndex, (row.columns?.length ?? 1) - 1);
  const selectedRegionId =
...
/** Add a cell only in the selected row (not in all rows), after the current column. */
function addCell(rowIndex, colIndex) {
  if (rowIndex < 0) return;
  const newRows = localModel.value.rows.map((r, i) => {
    if (i !== rowIndex) return r;
    const cols = r.columns && Array.isArray(r.columns) ? [...r.columns] : [];
    const insertIndex = colIndex >= 0 ? colIndex + 1 : cols.length;
    cols.splice(insertIndex, 0, { kind: "auto" });
  ...
  localModel.value = {
    ...localModel.value,
    rows: newRows,
    selectedRegionId: getRegionId(row.id, colIndex >= 0 ? colIndex + 1 : 0),
  };
}
...
<template>
  <div :class="isNestedTable ? 'h-100' : ''">
              ...
              >
                ...
                >
                  <button
                    ...
                    @click.stop="selectRegion(getRegionId(row.id, colIndex), 'column')"
                  >
                    <i class="fa-solid fa-gear"></i>
                  </button>
                </div>
                <div v-if="editMode" class="cell-action-buttons" @click.stop>
                  <div class="btn-group-vertical" role="group" aria-label="">
                    <button
                      ...
                      title="Cell Settings"
                      @click.stop="selectRegion(getRegionId(row.id, colIndex), 'cell')"
                    >
                      <i class="fa-solid fa-gear"></i>
                    </button>
                    <button
    ...
    <!-- Column Editor Offcanvas (Column Settings button). Teleport to body so it appears above parent table cells when nested. -->
    <Teleport to="body">
      <div
        class="offcanvas offcanvas-end offcanvas-column-editor"
        tabindex="-1"
        :class="{ show: isColumnSelected && selectedSettingsPanel === 'column' && editMode }"
        :aria-modal="isColumnSelected && selectedSettingsPanel === 'column' && editMode"
        :aria-labelledby="`column-editor-offcanvas-label-${offcanvasIdPrefix}`"
        :style="
          isColumnSelected && selectedSettingsPanel === 'column' && editMode
            ? {
                visibility: 'visible',
                zIndex: offcanvasZIndex,
                pointerEvents: 'auto',
      ...
      >
        ...
        >
          ...
          >
            ...
            {{
              selectedColumn
                ? `Row ${selectedColumn.rowIndex + 1}, Column ${selectedColumn.colIndex + 1}`
                : ""
            }}
          </h5>
          <button
              ...
              <div class="mt-3 pt-3 border-top">
                ...
                <div class="d-flex flex-column gap-2">
                  <div class="d-flex flex-column gap-2">
                    <div class="d-flex gap-2 align-items-center">
                        <button
                          class="btn btn-sm btn-outline-primary flex-grow-1 w-50"
                          @click="addColumn"
                        ...
                        <button
                          class="btn btn-sm btn-outline-danger flex-grow-1 w-50"
                          :disabled="
                            !selectedColumn ||
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        <button
                          class="btn btn-sm btn-outline-secondary flex-grow-1 w-50"
    ...
    <!-- Cell Editor Offcanvas (Cell Settings button). Teleport to body so it appears above parent table cells when nested. -->
    <Teleport to="body">
      <div
        class="offcanvas offcanvas-end offcanvas-cell-editor"
        tabindex="-1"
        :class="{ show: isCellSelected && editMode }"
        :aria-modal="isCellSelected && editMode"
        :aria-labelledby="`cell-editor-offcanvas-label-${offcanvasIdPrefix}`"
        :style="
          isCellSelected && editMode
            ? {
                visibility: 'visible',
                zIndex: offcanvasZIndex,
                pointerEvents: 'auto',
      ...
      >
        ...
        >
          <h5
            :id="`cell-editor-offcanvas-label-${offcanvasIdPrefix}`"
            class="offcanvas-title"
          >
            <i class="fa-solid fa-gear"></i> Settings:
            {{
              selectedCell
                ? `Row ${selectedCell.rowIndex + 1}, Cell ${selectedCell.colIndex + 1}`
                : ""
            }}
          </h5>
          <button
            ...
            class="btn-close"
            aria-label="Close"
            @click.stop="closeCellEditor"
          />
        </div>
        <div class="offcanvas-body">
          <div v-if="selectedCell" class="card">
            <div class="card-body">
              ...
              <div class="mb-3">
                <label
                  class="form-label"
                  :for="`cell-colspan-${offcanvasIdPrefix}-${selectedCell.rowIndex}-${selectedCell.colIndex}`"
                >
                ...
                <div class="d-flex gap-2 align-items-center">
                  <input
                    :id="`cell-colspan-${offcanvasIdPrefix}-${selectedCell.rowIndex}-${selectedCell.colIndex}`"
                    type="number"
                    class="form-control flex-grow-1"
                  ...
                  <div class="d-flex flex-column gap-2">
                    <div class="d-flex gap-2 align-items-center">
                        <button
                          ...
                          "
                        >
                          <i class="fa-regular fa-square-plus"></i> Cell
                        </button>
                        <button
                          class="btn btn-sm btn-outline-danger flex-grow-1 w-50"
                          :disabled="
                            !selectedCell ||
                            !selectedCell.allCells ||
                            selectedCell.allCells.length <= 1
                          "
                          @click="
                            removeCell(
                              selectedCell.rowIndex,
                          ...
                          "
                        >
                          <i class="fa-regular fa-square-minus"></i> Cell
                        </button>
                    </div>
                    <div class="d-flex gap-2 align-items-center">
                        <button
                          class="btn btn-sm btn-outline-secondary flex-grow-1 w-50"
                          :disabled="selectedCell.colIndex <= 0"
                          @click="
                            selectRegion(
                              getRegionId(
                                selectedCell.row.id,
...
/* Row action buttons - middle-left of first cell */
...
/* Column settings button - positioned top center */
...
/* Apply hover color when cell is hovered (but not if row settings button is selected) */
.grid-cell:hover
  .row-action-buttons
  .row-settings-btn:not(.row-settings-btn-selected) {
  background-color: #6c757d;
  border-color: #6c757d;
  color: #fff;
}
...
/* Keep red when cell is hovered and row settings button is selected */
.grid-cell:hover .row-action-buttons .row-settings-btn-selected {
  background-color: #dc3545;
  border-color: #dc3545;
  color: #fff;
}
...
/* Column settings button - same colors as cell action buttons */
.column-settings-button .cell-action-btn {
  background-color: #adb5bd;
  border-color: #adb5bd;
  color: #6c757d;
}

.column-settings-button .cell-action-btn:hover {
  background-color: #6c757d;
  border-color: #6c757d;
  color: #fff;
}
...
.column-settings-button .cell-action-btn-selected {
  background-color: #0d6efd;
  border-color: #0d6efd;
  color: #fff;
}

.column-settings-button .cell-action-btn-selected:hover {
  background-color: #0d6efd;
  border-color: #0d6efd;
  color: #fff;
}

.grid-cell:hover .column-settings-button .cell-action-btn-selected {
  background-color: #0d6efd;
  border-color: #0d6efd;
  color: #fff;
}
...
/* Hover state for selected column (Column Settings): maintain Bootstrap primary blue */
.grid-cell[data-column-selected="true"]:hover {
  border-color: #0d6efd !important;
}
